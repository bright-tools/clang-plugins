...
@test "Check that a violation of the child path check is detected" {
    run ${CLANG_EXE} -c -fplugin=${PLUGIN_TARGET} includes_via_child_path.c includes_via_parent_path.c -Xclang -plugin-arg-include-path-checker -Xclang -disallow-child-dir-include-references -Xclang -plugin-arg-include-path-checker -Xclang -disallow-parent-dir-include-references
    assert_failure
